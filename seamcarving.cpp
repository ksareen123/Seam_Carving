...
int main(int argc, char *argv[])
{
  ...
  if (!hasError)
  {
    /*
    // FIXME
    int* seam_test = findMinVerticalSeam(image, width, height);
    getVerticalSeam(image, width - 1, width, height, seam_test);
    for (int i = 0; i < height; i++) {
      cout << seam_test[i] << endl;
    }
    
    if (getVerticalSeam(image, width - 1, width, height, seam_test) != 126528) {
      cout << "VERTICAL SEAM: INCORRECT" << endl;
    }
    else {
      cout << "VERTICAL SEAM: CORRECT" << endl;
    }
    // FIXME
    */
